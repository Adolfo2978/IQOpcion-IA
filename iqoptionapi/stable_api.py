...
class IQ_Option:
    ...
    def get_all_init_v2(self):
        ...
        while self.api.api_option_init_all_result_v2 == None:
            if time.time() - start_t >= 10:
                logging.warning('**warning** get_all_init_v2 late 10 sec')
                return None
    ...
    def get_all_open_time(self):
        ...
        if not getattr(self, "disable_digital_open_time", False):
            digital_payload = self.get_digital_underlying_list_data()
            try:
                digital_data = digital_payload["underlying"] if digital_payload else None
            except Exception:
                digital_data = None
            if digital_data:
    ...
    def start_candles_stream(self, ACTIVE, size, maxdict):
        ...
        if size == "all":
            ...
            return self.start_candles_all_size_stream(ACTIVE)
    ...
    def start_candles_one_stream(self, ACTIVE, size):
        ...
        while True:
            if time.time() - start > 20:
                logging.error(
                    '**error** Tiempo de espera agotado (20s) al iniciar stream de velas (start_candles_one_stream)')
                return False
            try:
    ...
    def buy_digital_spot(self, active, amount, action, duration):
        ...
        start_t = time.time()
        while True:
            result = self.api.digital_option_placed_id
            if isinstance(result, dict) and request_id in result:
                placed = result.get(request_id)
